...
function generateQRPayload() {
  ...
  const payload = {
    type: 'TUB_TICKET',
    userId: localUser?.id || 'unknown',
    nome: activeTicket.value.titular,
    bilhete: activeTicket.value.tipo,
    nif: activeTicket.value.nif,
    ts,
    nonce
  }
    ...
    <div class="ticket-card">
      ...
      <div class="qr-section">
        ...
        <div class="qr-container" :class="{'qr-refreshing': qrExpired}">
          <img v-if="qrDataUrl" :src="qrDataUrl" alt="QR Code do bilhete" class="qr-image" />
          <div v-else class="qr-loading">A gerar QR…</div>
        </div>
